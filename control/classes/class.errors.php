...
define('FILE_ERR_LOG_TXT', 'ms_error_log.log'); // File name of error log
define('FILE_FATAL_ERR_LOG_TXT', 'ms_fatal_error_log.log'); // File name of fatal error log
...
function msFatalErr() {
  ...
  if ($error['type'] == E_ERROR || $error['type'] == 4) {
    ...
    if (ERR_HANDLER_DISPLAY) {
      echo '<div style="background:#ff9999"><p style="padding:10px;color:#fff">A fatal error has occurred. For more details please view "' . ERR_HANDLER_LOG_FOLDER . '/' . FILE_FATAL_ERR_LOG_TXT . '".</div>';
    }
...
function msErrorhandler($errno, $errstr, $errfile, $errline) {
  ...
  switch ($errno) {
    ...
    case E_USER_WARNING:
      ...
      $string .= '[Error on line ' . $errline . ' in file ' . $errfile . ']';
      if (ERR_HANDLER_DISPLAY) {
        echo '<div style="background:#ff9999"><p style="padding:10px;color:#fff">An error has occurred. For more details please view "' . ERR_HANDLER_LOG_FOLDER . '/' . FILE_ERR_LOG_TXT . '".</div>';
      }
